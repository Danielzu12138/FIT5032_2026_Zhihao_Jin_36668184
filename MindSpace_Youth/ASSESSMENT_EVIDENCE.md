...
| --- | --- |
| D.1 External authentication | Firebase Email/Password Authentication in `src/firebase.js` and `src/utils/auth.js`, including password reset. Registration uses a Resend OTP and creates a Firebase account only after the address is verified. |
...
| Requirement | Implementation evidence |
| --- | --- |
| E.1 Cloud Functions | Cloudflare Pages Functions expose health, resources, registration OTP, booking validation and email routes under `functions/api`. The registration Functions use KV, Resend, Firebase Auth REST and Firestore REST; protected routes verify Firebase ID token signatures and Firestore roles. |
...
6. Verified registration: expiring, rate-limited OTP challenges are stored as HMAC hashes in Cloudflare KV, and user records are created only after successful proof of email access.
